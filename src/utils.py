...
from config import JSON_SAVER_PATH, CSV_SAVER_PATH
from src.funcs_for_save import open_json_file, open_csv_file


def make_salary(salary: dict) -> str:
    """
...
def convert_vacancies_to_list(vacancies):
    """
    ...
    if vacancies.get('items'):
        for vacancy in vacancies['items']:
            format_vacancy = {'vacancy': vacancy['name'], 'url': f"https://hh.ru/vacancy/{vacancy['id']}",
        ...
        for vacancy in vacancies['objects']:
            vacancy['salary'] = {'from': vacancy['payment_from'], 'to': vacancy['payment_to'],
                                 'currency': vacancy['currency']}
            format_vacancy = {'vacancy': vacancy['profession'], 'url': vacancy['link'],
                              'salary': make_salary(vacancy['salary']),
...
def get_vacancy_by_index(convert_top_vacancies, index):
    """
...
def concatenate_dicts(json_file, csv_file):
    """
...
def delete_vacancy(vacancies, index):
    """
